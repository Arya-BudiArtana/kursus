...
@section('content')
    <div class="px-4 container-fluid">
        <h1 class="mt-4">Edit Peminjaman Kendaraan</h1>
        ...
        <div class="mb-4 card">
            <div class="card-header">
                Formulir Edit Peminjaman Kendaraan
            </div>
            <div class="card-body">
                ...
                @endif
                <form method="POST" action="{{ route('pegawai.peminjaman-kendaraan.update', $data_sebelumnya->id) }}">
                    @csrf
                    @method('put')
                    <div class="fv-row mb-7">
                        ...
                        <select required class="form-select" name="id_kendaraan" id="id_kendaraan"
                            ...
                            @foreach ($data_kendaraan as $item)
                                <option value="{{ $item->id }}" @selected(old('id_kendaraan', $data_sebelumnya->id_kendaraan) == $item->id)>
                                    {{ $item->type_kendaraan }} </option>
                            @endforeach
                        </select>
                        <!--end::Input-->
                    </div>
