                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($item->approval == 'draft')
                                        <form action="{{ route('admin.peminjaman-kendaraan.approvePinjaman', $item->id) }}"
                                            method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-success btn-sm"
                                                onclick="return confirm('Yakin ingin mengapprove data ini?')">
                                                <i class="fas fa-check"></i> Approve
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.peminjaman-kendaraan.tolakPinjaman', $item->id) }}"
                                            method="POST" class="d-inline">
                                            @csrf
